...
for (const theme of [darkTheme, lightTheme]) {
  describe(`${theme.mode} theme contrast`, () => {
    const { cron, uptime } = timelineStylesFor(theme);
    const backgrounds = [
    ...
    for (const [name, colors] of [
      ...
      for (const [status, color] of Object.entries(colors)) {
        test(`timeline ${name}.${status} separates from the unlit track`, () => {
          expect(contrastRatio(color, cron.trackColor)).toBeGreaterThanOrEqual(2);
        });
      }
    }
    ...
    test("the timeline's unlit track is visible but recedes", () => {
      expect(contrastRatio(cron.trackColor, theme.bg)).toBeGreaterThanOrEqual(1.3);
      expect(contrastRatio(cron.trackColor, theme.bg)).toBeLessThan(AA_LARGE);
    });
    ...
    test("okay and failed are not mistakable for each other", () => {
      ...
      for (const [good, bad] of [
        [cron.colors.ok, cron.colors.error],
        [uptime.colors.success, uptime.colors.failure_incident],
      ]) {
