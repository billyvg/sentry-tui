...
/** Run a command, returning captured output and the exit code, printing nothing. */
async function capture(command: string[]): Promise<{ code: number; out: string; err: string }> {
...
async function preflight(): Promise<void> {
  ...
  if (plan.pendingCommit) {
    console.log(`  resume pending release commit ${plan.pendingCommit.slice(0, 7)}`);
  }
...
interface ReleasePlan {
  ...
  versions: ReleaseVersions;
  /** Existing version commit left by an interrupted release, if resuming. */
  pendingCommit?: string;
}

/** The exact subject written by the remote version-bump commit. */
function releaseCommitMessage(
  components: readonly ReleaseComponent[],
  versions: ReleaseVersions,
): string {
  return `chore: release ${components
    .map((component) => `${component} v${versions[component]}`)
    .join(", ")}`;
}

/** Recognize a release commit whose versions still match the current manifest. */
export function pendingReleaseComponents(
  message: string,
  versions: ReleaseVersions,
): ReleaseComponent[] | undefined {
  const headline = message.split("\n", 1)[0];
  for (const components of [["host"], ["app"], ["host", "app"]] as const) {
    if (headline === releaseCommitMessage(components, versions)) return [...components];
  }
  return undefined;
}

/** A component tag for this version, including the legacy unified tag. */
async function existingReleaseTag(
  component: ReleaseComponent,
  version: string,
...
/**
 * Find a version commit that succeeded just before its workflow dispatch did not.
 *
 * The commit message and one-file diff are both generated by this command, so
 * together they distinguish a resumable cut from an arbitrary manifest edit.
 */
async function pendingRelease(
  source: RemoteReleaseSource,
): Promise<{ components: ReleaseComponent[]; commit: string } | undefined> {
  const history = await capture([
    "gh",
    "api",
    `repos/${REPOSITORY}/commits?sha=${encodeURIComponent(source.branch)}&path=release.json&per_page=1`,
  ]);
  if (history.code !== 0) return undefined;

  let latest: { sha?: string; commit?: { message?: string } } | undefined;
  try {
    [latest] = JSON.parse(history.out || "[]") as Array<{
      sha?: string;
  ...
  if (!latest?.sha || !latest.commit?.message) return undefined;

  const components = pendingReleaseComponents(latest.commit.message, source.versions);
  if (!components) return undefined;

  const changed = await capture([
    "gh",
    "api",
    `repos/${REPOSITORY}/commits/${latest.sha}`,
    "--jq",
    ".files[].filename",
  ]);
  if (changed.code !== 0 || changed.out !== "release.json") return undefined;

  for (const component of components) {
    if (await existingReleaseTag(component, source.versions[component])) return undefined;
  }
  return { components, commit: latest.sha };
}
...
async function releasePlan(source: RemoteReleaseSource): Promise<ReleasePlan> {
  const requested = explicitlyRequestedComponents();
  const pending = await pendingRelease(source);
  if (pending) {
    if (
      requested &&
      (requested.length !== pending.components.length ||
        requested.some((component) => !pending.components.includes(component)))
    ) {
      die(
        `release ${pending.components.join(" + ")} is already pending at ${pending.commit.slice(0, 7)}; ` +
          "re-run without different component selectors",
      );
    }
    if (positionals.length > 0 || requestedVersionBumps(process.argv.slice(3)).length > 0) {
      die(
        `release ${pending.components.join(" + ")} is already pending at ${pending.commit.slice(0, 7)}; ` +
          "re-run without a version selector",
      );
    }
    return {
  ...
  const components = requested ?? (await inferredReleaseComponents(source));
...
async function createReleaseCommit(
  ...
  const commit = await capture([
    ...
    `head=${source.head}`,
    "-f",
    `message=${releaseCommitMessage(plan.components, plan.versions)}`,
    "-f",
    "path=release.json",
    "-f",
...
async function dispatchRelease(
  ...
  const dispatched = await capture(
    releaseDispatchCommand(plan.components, plan.versions, commit, requestId),
  );
  if (dispatched.code !== 0) {
    const detail = dispatched.err || dispatched.out;
    die(`could not dispatch the Release workflow${detail ? `: ${detail}` : ""}`);
  }
}

/** Build the repository-dispatch invocation, with one field flag per value. */
export function releaseDispatchCommand(
  components: readonly ReleaseComponent[],
  versions: ReleaseVersions,
  commit: string,
  requestId: string,
  ...
  return [
    ...
    "-f",
    `client_payload[components]=${components.join(",")}`,
    "-f",
    `client_payload[host_version]=${versions.host}`,
    "-f",
    `client_payload[app_version]=${versions.app}`,
    "-f",
...
async function cut(): Promise<void> {
  ...
  const releaseSource = plan.pendingCommit ?? source.head;
  if (flags.force) warn("--force: releasing without checking CI");
  else await requireGreenCi(releaseSource);

  step(
    `${plan.pendingCommit ? "Resuming" : "Releasing"} ${plan.components.join(" + ")} from GitHub`,
  );
  console.log(`  ${REPOSITORY} ${source.branch}@${releaseSource.slice(0, 7)}`);
  for (const component of plan.components) {
    console.log(
  ...
  let releaseCommit = releaseSource;
