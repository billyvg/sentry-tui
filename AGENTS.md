...
Platform icons in `src/assets/platform-icons/` and the lookup table in
`src/lib/platformIcons.generated.ts` are both generated by `bun run icons:build`
from the `platformicons` package. Both are committed; rerun only when that
package is upgraded.
