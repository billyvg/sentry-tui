...
One tag drives every channel. Push `v0.1.0` and CI builds a runtime host per
platform plus one shared app payload, publishes the npm packages, records the
successful production deployment, and creates the GitHub Release from the same
assets.
...
artifacts into `dist/bin`, builds the shared payload, re-runs
`build:npm --strict`, shows you what it is about to publish, and then publishes
in dependency order: platform packages, app payload, launcher, alias.
`--skip-download` reuses whatever is already in `dist/bin`.
...
With 2FA on writes, pass `--otp`. Seven uploads can outlive one 30-second code
