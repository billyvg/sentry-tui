...
app. The payload owns the replaceable application tree. Both tiers feed npm and
each GitHub Release archive contains the matching host and payload.
...
`src/ui/runtime/loadPayload.ts`. This identity is load-bearing: bundling another
...
The npm launcher runs the newest cached host and payload in
`~/.cache/sentry-tui/versions` (or the npm-installed pair) straight away. Once
that child has exited, it spawns `packaging/npm/background-update.mjs` detached
to fetch a newer payload. The worker must never write to stdout or stderr — a
...
`HOST_API_VERSION` in `src/app/runtimeContract.ts` is the compatibility gate.
